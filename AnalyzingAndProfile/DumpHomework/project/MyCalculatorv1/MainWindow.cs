...
public partial class MainWindow : Window, IComponentConnector
{
    ...
    {
        if (IsExpressionValid(tb.Text))
            result();
        else
            tb.Text = "Invalid input. Try again!";

    }

    private bool IsExpressionValid(string expression)
    {
        string sanitizedExpression = Regex.Replace(expression, @"\s+", "");

        string pattern = @"^[-+*/()\d]+$";

        Match match = Regex.Match(sanitizedExpression, pattern);

        if (!match.Success)
            return false;

        if (Regex.IsMatch(sanitizedExpression, @"[*/+-]{2,}"))
            return false;
        return true;
    }
